...
import { ArnPrincipal, CompositePrincipal, ManagedPolicy, PolicyDocument, PolicyStatement, Role, ServicePrincipal } from 'aws-cdk-lib/aws-iam';
...
export class CicdPipelineStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const cicdPipelineConfig = CdkUtil.getEnvConfig('cicdPipeline');

    // S3 Buckets
    // https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_s3.Bucket.html
    const artifactBucketName = CdkUtil.getResourceName('cicd-artifact');
    const artifactBucket = new Bucket(this, artifactBucketName.constructId(), {
      bucketName: artifactBucketName.kebabCase(),
      autoDeleteObjects: true,
      removalPolicy: RemovalPolicy.DESTROY,
    });
    const deployDestBucketName = CdkUtil.getResourceName('cicd-deploy-dest');
    const deployDestinationBucket = new Bucket(this, deployDestBucketName.constructId(), {
    ...
    const connectionName = CdkUtil.getResourceName('examining-conn');
    if (connectionName.kebabCase().length > 32) {
      throw new Error(`The name length of a CodeStar connection cannot exceed 32 characters. connectionName: ${connectionName.kebabCase()}`);
    }
    const connection = new CfnConnection(this, connectionName.constructId(), {
    ...
    const pipelineName = CdkUtil.getResourceName('examining-cicd-with-github');
    const pipeline = new Pipeline(this, pipelineName.constructId(), {
  ...
  private createIAMRoleForPipeline(namePrefix: string, params: { connectionArn: string }): Role {
    ...
    const iamPolicyDocumentForPipeline = new PolicyDocument({
      statements: [
        ...
        new PolicyStatement({
          ...
          resources: [`arn:aws:cloudformation:${CdkUtil.E.REGION_NAME}:${CdkUtil.E.ACCOUNT_ID}:stack/${CdkUtil.E.PJ_CODE_NAME}-${CdkUtil.E.ENV_NAME}-*`],
        }),
        new PolicyStatement({
    ...
    CdkUtil.debugOut.writeJsonFile(outFileName, iamPolicyJson);

    // Create the IAM Role for CodePipeline pipelines.
    const iamRoleForPipelineName = CdkUtil.getResourceName(`${namePrefix}-role`);
    const iamRoleForPipeline = new Role(this, iamRoleForPipelineName.constructId(), {
  ...
  private createIAMRoleForStages(
    ...
    // - Source stage
    const iamPolicyForSourceStageName = CdkUtil.getResourceName(`${namePrefix}-source-policy`);
    const iamPolicyForSourceStage = new ManagedPolicy(this, iamPolicyForSourceStageName.constructId(), {
    ...
    // - Deploy stage
    const iamPolicyForDeployStageName = CdkUtil.getResourceName(`${namePrefix}-deploy-policy`);
    const iamPolicyForDeployStage = new ManagedPolicy(this, iamPolicyForDeployStageName.constructId(), {
    ...
    const iamRoleForStagesName = CdkUtil.getResourceName(`${namePrefix}-role`);
    const iamRoleForStages = new Role(this, iamRoleForStagesName.constructId(), {
